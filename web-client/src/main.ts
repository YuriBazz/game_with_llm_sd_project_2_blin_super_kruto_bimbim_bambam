...
import { isVictory, isDefeat, isLevelComplete, isPlayable, type GameState } from './types/game';
...
async function pickupItem(): Promise<void> {
    ...
    try {
        ...
        if (response.state) {
            ...
            await syncVisibleCells(response.state);
            if (response.success) {
                gameLog.add('Picked up item');
            } else {
                gameLog.add('Nothing to pickup here');
            }
        }
        render();
    } catch (error) {
...
function updateSpectatorBanner(): void {
    ...
    if (store.state?.god_mode) {
        ...
        banner.textContent = 'SPECTATOR — you are O';
    } else {
